...
`npm run demo:comprehensive:auto` runs the non-interactive smoke path for CI. For public demo prep, run `npm run demo:comprehensive`; it invokes each lane, opens browser surfaces, assumes real Ledger validation unless the reviewer types `demo`, and captures reviewer observations in the combined receipt.
